...
const AdPlacement: React.FC<AdPlacementProps> = ({
  ...
  const getAdConfig = () => {
    const slots = {
      header: {
        slot: customSlot || process.env.NEXT_PUBLIC_ADSENSE_HEADER_SLOT || '1234567890',
        format: 'horizontal' as const,
        style: { width: '100%', height: '90px' },
        className: 'mb-4'
      },
      sidebar: {
        slot: customSlot || process.env.NEXT_PUBLIC_ADSENSE_SIDEBAR_SLOT || '1234567891',
        format: 'vertical' as const,
        style: { width: '300px', height: '600px' },
        className: 'hidden lg:block'
      },
      content: {
        slot: customSlot || process.env.NEXT_PUBLIC_ADSENSE_CONTENT_SLOT || '1234567892',
        format: 'rectangle' as const,
        style: { width: '100%', height: '250px' },
        className: 'my-8'
      },
      footer: {
        slot: customSlot || process.env.NEXT_PUBLIC_ADSENSE_FOOTER_SLOT || '1234567893',
        format: 'horizontal' as const,
        style: { width: '100%', height: '90px' },
        className: 'mt-8'
      },
      'mobile-banner': {
        slot: customSlot || process.env.NEXT_PUBLIC_ADSENSE_MOBILE_SLOT || '1234567894',
        format: 'auto' as const,
        style: { width: '100%', height: '50px' },
        className: 'block lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-white shadow-lg'
      }
    };
  ...
  return (
    <div className={`ad-placement ad-placement-${position} ${adConfig.className}`}>
...
export const HeaderAd: React.FC<{ enabled?: boolean }> = ({ enabled }) => (
...
export const MobileBannerAd: React.FC<{ enabled?: boolean }> = ({ enabled }) => (
